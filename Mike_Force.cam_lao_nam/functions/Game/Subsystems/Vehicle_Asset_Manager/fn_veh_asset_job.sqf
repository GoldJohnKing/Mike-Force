/*
	...
	Description:
		Scheduler job that handles the vehicle respawn subsystem.
...
private _fnc_processVehicle = {
	...
	_vehicleInfo select struct_veh_asset_info_m_respawn_type params ["_respawnType", "_respawnTime"];
	...
	if ((call _fnc_getState select _state) == "WRECKED") then {
		...
		if (isNull _vehicle) then {
			...
			[_id] call vn_mf_fnc_veh_asset_add_package_action;
		};
	...
	if ((call _fnc_getState select _state) in ["RESPAWNING", "REPAIRING"] && {(call _fnc_getState select 1) < serverTime}) then {
		[_id] call vn_mf_fnc_veh_asset_respawn;
	};
};
