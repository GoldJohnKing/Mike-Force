[] spawn {
	...
	// if (isClass (configfile >> "CfgPatches" >> "cba_keybinding")) then { // Edited: Force using action menu instead of CBA Keybind
	// 	call CHVDAddon_fnc_keyBindings;
	// } else {
		_actionText = if (isLocalized "STR_chvd_title") then {localize "STR_chvd_title"} else {"View Distance Settings"};
		player addAction [_actionText, CHVDAddon_fnc_openDialog, [], -99, false, true, '', '_target isEqualTo _this'];
		player addEventHandler ["Respawn", format ["player addAction ['%1', CHVDAddon_fnc_openDialog, [], -99, false, true, '', '_target isEqualTo _this']", _actionText]];
	// };
	...
	[] spawn {
		for "_i" from 0 to 1 step 0 do {
			...
			if (
				...
				[1] call CHVDAddon_fnc_updateSettings;
				[] call CHVDAddon_fnc_updateTerrain;
				[4] call CHVDAddon_fnc_updateSettings
			} else {				
				[] call CHVDAddon_fnc_updateSettings;
			};
		};
	...
	[] spawn {
		for "_i" from 0 to 1 step 0 do {
			...
			if (
				...
				(CHVD_vehType isEqualTo 1 && CHVD_carSyncMode isEqualTo 2) || 
				(CHVD_vehType isEqualTo 2 && CHVD_airSyncMode isEqualTo 2)
			) then {[4] call CHVDAddon_fnc_updateSettings};
		};
	};
};